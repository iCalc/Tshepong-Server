...
namespace Tshepong
{
    public partial class scrTeamD : Form
    {
        #region Declarations
        int columnnr = 0;
        int intNoOfDays = 0;
        ...
        StringBuilder _strSqlAlter = new StringBuilder();
        int _intCounter = 0;
        ...
        internal void scrTeamDLoad(string Period, string Region, string BussUnit, string Userid, string MiningType, string BonusType, string Environment)
        {
            ...
            lblEnvironment.Text = Environment.Trim();
            txtDatabaseName.Text = "DEVTM4000";
            ...
            buttonCollection["tabDrillPen"] = btnLockDrillPen;
            buttonCollection["GanglinkEARN10"] = btnBaseCalcs;
            buttonCollection["GanglinkEARN40"] = btnGangLinkCalcs;
            buttonCollection["SupportLinkEARN20"] = btnSupportLinkCalc;
            buttonCollection["DrillersEARN30"] = btnDrillerCalcs;
            buttonCollection["Bonus Report Process - Phase 1"] = btnBonusPrints;
            buttonCollection["BonusShiftsEARN60"] = btnBonusShiftsCalcs;
            buttonCollection["Bonus Report Process - Phase 1"] = btnBonusPrints;
            buttonCollection["MinersEARN60"] = btnMinersCalc;
        ...
        private void setEnvironment()
        {
            ...
            Base.StopeDatabaseName = "STPTM4000";
            Base.DevDatabaseName = "DEVTM4000";
            ...
            //string strPath = "z:\\icalc\\Harmony\\Phakisa\\" + strServerPath + "\\REPORTS\\" + strdb.Replace(BusinessLanguage.Period, "").Replace("4000", "Conn") + ".udl";
            ...
            if (myConn.State == ConnectionState.Open)
            {
                ...
                switch (lstTableNames.Count)
                {
                    ...
                    default:
                        ...
                        {
                            if (s.ToString().IndexOf("EARN") > 0)
                            {
                                if (s.ToString().IndexOf(BusinessLanguage.Period) > 0)
                                {
                                    lstbox.Items.Add(s);
                                }
                            }
        ...
        private void loadLapaShifts()
        {
            ...
            {
                //YES
                string strSQL = "Select * from OFFDAYS " + strWhere + " and LapaShiftInd = 'Y'";
                LapaShifts = TB.createDataTableWithAdapter(Base.DBConnectionString, strSQL);

            }
            else
            {
                 
                LapaShifts = TB.createDataTableWithAdapter(Base.DBConnectionString, "Select * from OFFDAYS");
        ...
        private void evaluateSurvey()
        {
            ...
            if (intCount > 0)
            {
                ...
                if (Survey.Rows.Count == 0)
                {
                    strSQL = "SELECT * from SURVEY " + strWhere;
        ...
        private void evaluateLabour()
        {
            ...
            if (intCount > 0)
            {
                ...
                //string strAllNames = "select * from bonusshifts where section = 115";
                ...
                foreach (DataRow data in Workers.Rows)
                {
                    ...
                    cboNames.Items.Add(data["EMPLOYEE_NO"].ToString().Trim());
                }

                IEnumerable<DataRow> query1 = from locks in Workers.AsEnumerable()
                                              where locks.Field<string>("Wagecode").Trim() == "316M002"
                                              select locks;

                try
                {
                    DataTable shiftbosses = query1.CopyToDataTable<DataRow>();

                    if (shiftbosses.Rows.Count > 0)
                    {
                        lstNames = TB.loadDistinctValuesFromColumn(shiftbosses, "EMPLOYEE_NAME");

                        cboGanglinkShiftbosses.Items.Clear();
                        foreach (string s in lstNames)
                        {

                            cboGanglinkShiftbosses.Items.Add(s.Trim());
        ...
        private void hideColumnsOfGrid(string gridname)
        {
            ...
            switch (gridname)
            {
                ...
                case "grdSupportLink":
                    #region grdSupportLink
                    if (grdSupportLink.Columns.Contains("BUSSUNIT"))
                    {
                        this.grdSupportLink.Columns["BUSSUNIT"].Visible = false;
                    }
                    if (grdSupportLink.Columns.Contains("MININGTYPE"))
                    {
                        this.grdSupportLink.Columns["MININGTYPE"].Visible = false;
                    }
                    if (grdSupportLink.Columns.Contains("BONUSTYPE"))
                    {
                        this.grdSupportLink.Columns["BONUSTYPE"].Visible = false;
                    }
                    return;
                    #endregion
        ...
        private void loadDrillers()
        {
            ...
            if (intCount > 0)
            {
                ...
                Drillers = TB.createDataTableWithAdapter(Base.DBConnectionString,
                           "SELECT * FROM DRILLERS WHERE SECTION = '" + txtSelectedSection.Text.Trim() + "'AND PERIOD = '" + txtPeriod.Text.Trim() + "'");
                txtAutoDGang.Clear();
            }
            else
            {
        ...
        private void UpdateClockedShifts()
        {
            ...
            IEnumerable<DataRow> query1 = from locks in Offdays.AsEnumerable()
                                          where locks.Field<string>("GANG").TrimEnd() == "DUMMY"
                                          where locks.Field<string>("PERIOD").TrimEnd() == BusinessLanguage.Period.Trim()
                                          select locks;

            try
            {
                DataTable sectionOffdays = query1.CopyToDataTable<DataRow>();


                #region Appply offdays to clockedshifts
                if (sectionOffdays.Rows.Count > 0)
                {
                    foreach (DataRow dr in sectionOffdays.Rows)
                    {
            ...
            #region Appply offdays to lapashifts
                foreach (DataRow lap in LapaShifts.Rows)
                {
                    string Lapaoffdate = lap["OFFDAYVALUE"].ToString();
                    if (Lapaoffdate.Trim() == "2009-01-01")
                    {
                    }
                    else
                    {

                        DateTime dteOffdate = Convert.ToDateTime(lap["OFFDAYVALUE"].ToString());
                        int intOffday = 0;
                        ...
                        Base.UpdateLapaShiftsPerGang(Base.DBConnectionString, intOffday,
                                 " Where GANG = '" + lap["GANG"].ToString().Trim() +
                                 "' and Period = '" + BusinessLanguage.Period + "' ");
        ...
        private void evaluateGangLinking()
        {
            ...
            if (lstNames.Count > 1)
            {
                ...
                foreach (string s in lstNames)
                {
                    ...
                    lstSupportGangs.Items.Add(s.Trim());
                }
            }
            else
            {
                lstNames = TB.loadDistinctValuesFromColumn(Clocked, "GANG");
                if (lstNames.Count > 1)
                {
                    foreach (string s in lstNames)
                    {
        ...
        private void btnImportSurvey_Click(object sender, EventArgs e)
        {
            ...
            if (Status.Rows.Count > 0)
            {
                IEnumerable<DataRow> query1 = from locks in Status.AsEnumerable()
                                              where locks.Field<string>("SECTION").Trim() == txtSelectedSection.Text.Trim()
                                              where locks.Field<string>("PROCESS").Trim() == "tabSurvey"
                ...
                {
                    MessageBox.Show("No records on status for the selected section");
                }
            }
            else
            {
        ...
        private void refreshSurvey()
        {
            ...
            string FilePath_XLSX = "C:\\iCalc\\Harmony\\Tshepong\\" + strServerPath + "\\Data\\productionDEV_" + BusinessLanguage.Period.Trim() + ".xlsx";
            string FilePath_XLS = "C:\\iCalc\\Harmony\\Tshepong\\" + strServerPath + "\\Data\\productionDEV_" + BusinessLanguage.Period.Trim() + ".xls";
            string FilePath = "";
            ...
            {
                FilePath = "C:\\iCalc\\Harmony\\Tshepong\\" + strServerPath + "\\Data\\productionDEV_" + BusinessLanguage.Period.Trim() + ".xls";
            }

            if (XLSX_exists.Equals(true))
            {
                FilePath = "C:\\iCalc\\Harmony\\Tshepong\\" + strServerPath + "\\Data\\productionDEV_" + BusinessLanguage.Period.Trim() + ".xlsx";
            }
            ...
            */
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                //{
                //    if ((row["SECTION"].ToString().Trim() == txtSelectedSection.Text.Trim()))
                //    {
                //    }
                //    else
                //    {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    row["MININGTYPE"] = "DEVELOPMENT";
                    row["BONUSTYPE"] = "TEAM";
                    //if (row["GANG"].ToString().Length < 4)
                    //{
                    //    row["SECTION"] = "XXXXXX";
                    //}
                    //else
                    //{
                    //    row["SECTION"] = row["GANG"].ToString().Substring(0, 4);
                    //}
                ...
                if (intCount > 0)
                {
                    ...
                    btnx.Enabled = true;
                    btnx.Text = "Run";
                    TB.deleteProcess(Base.AnalysisConnectionString, Base.DBName + BusinessLanguage.Period);  //xxxxxxxxxxxxxxxx
        ...
        {
            string strSQL = "select SECTION, PERIOD, WORKPLACE, 'XXX' as EMPLOYEE_NO, '1' as DESIGNATION, '0' as SHIFTS_WORKED, '0' as AWOP_SHIFTS,'0' as PAYSHIFTS,'0' as SAFETYIND " +
                            "from Survey  ";
        ...
        //    string strSQL = "select distinct t1.SECTION, t1.PERIOD, t1.GANG, 'XXX' as CONTRACT, 'XXX' as WORKPLACE, '0' as SAFETYIND, 'XXX' as GANGTYPE,  " +
        //                    "substring(t1.GANG,1,12) + substring(t1.GANG,15,1) as CREWNO,t2.GANGSHIFTS from BONUSSHIFTS as t1, " +
        //                    " (select gang,sum(convert(float,Employee_Labourshifts)) as GANGSHIFTS from bonusshifts where substring(gang,10,3) in " + strActivity +
        //                    " and substring(gang,6,2) in " + strMO + " group by gang) as t2 where t1.gang = t2.gang ";
        ...
        private void btnImportADTeam_Click_Old(object sender, EventArgs e)
        {
        ...
        private void refreshLabour2()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                string strSQL = "Select *,'0' as SHIFTS_WORKED,'0' as AWOP_SHIFTS, '0' as LAPA_SHIFTS," +
        ...
        private void refreshLabour()
        {
            ...
            bool XLSX_exists = File.Exists("C:\\iCalc\\Harmony\\Tshepong\\" + strServerPath + "\\Data\\masterdev" + BusinessLanguage.Period.Trim() + ".xlsx");
            bool XLS_exists = File.Exists("C:\\iCalc\\Harmony\\Tshepong\\" + strServerPath + "\\Data\\masterdev" + BusinessLanguage.Period.Trim() + ".xls");

            if (XLSX_exists.Equals(true))
            {
                //MessageBox.Show("nou in xlsx filepath");
                string status = excel.OpenFile("C:\\iCalc\\Harmony\\Tshepong\\" + strServerPath + "\\Data\\masterdev" + BusinessLanguage.Period.Trim() + ".xlsx", "");
            ...
            if (XLS_exists.Equals(true))
            {
                ...
                string status = excel.OpenFile("C:\\iCalc\\Harmony\\Tshepong\\" + strServerPath + "\\Data\\masterdev" + BusinessLanguage.Period.Trim() + ".xls", "");
            ...
            //MessageBox.Show("gaan nou die sheetnames kry");
            string[] sheetNames = GetExcelSheetNames(FilePath);
        ...
        private void grdMiners_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            ...
            string test = scrTeamD.ActiveForm.ActiveControl.Name;
        ...
        private void btnInsertRow_Click(object sender, EventArgs e)
        {
            ...
            switch (tabInfo.SelectedTab.Name)
            {
                ...
                case "tabGangLinking":
                        ...
                        cboGangLinkGang.Text.Trim().Length > 0)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                DataRow dr = temp.NewRow();
                                strGang = lstGangs.SelectedItems[i].ToString().Trim();
                                dr["BUSSUNIT"] = BusinessLanguage.BussUnit.Trim();
                                dr["MININGTYPE"] = BusinessLanguage.MiningType.Trim();
                                dr["BONUSTYPE"] = BusinessLanguage.BonusType.Trim();
                                dr["SECTION"] = txtSelectedSection.Text.Trim();
                                ...
                                dr["SHIFTBOSS"] = cboGanglinkShiftbosses.Text.Trim();
                            ...
                            TB.InsertData(Base.DBConnectionString, "Update ganglink set shiftboss = '" + cboGanglinkShiftbosses.Text.Trim() +
                                                                  "' where gang = '" + strGang + "'");
                            evaluateGangLinking();
                ...
                case "tabEmplPen":
                    ...
                    if (cboEmplPenEmployeeName.Text.Trim().Length > 0 &&
                    ...
                    if (cboOffDaysSection.Text.Trim().Length > 0)
                    {
                        ...
                        else
                        {
                            ...
                            for (int i = 0; i < lstOffDayValue.SelectedItems.Count; i++)
                            {
                                ...
                                dr["LAPASHIFTIND"] = cboLapaShiftInd.Text.Trim();
                ...
                case "tabSupportLink":
                    #region tabSupportLink

                    if (cboSupportGangType.Text.Trim().Length > 0 &&
                        txtSupportLinkDayShiftGang.Text.Trim().Length > 0)
                    {

                        //Get the layout of the ganglink file.
                        DataTable temp = new DataTable();
                        temp = SupportLink.Copy();

                        int intRow = 0;
                        if (grdSupportLink.CurrentRow == null)
                        {
                            intRow = 0;
                        }
                        else
                        {
                            intRow = grdSupportLink.CurrentCell.RowIndex;
                        }
                        ...
                        if (lstSupportGangs.SelectedItems.Count == 0)
                        {
                            MessageBox.Show("Please select gangs from the listbox", "Information", MessageBoxButtons.OK);
                        }
                        else
                        {
                            if (cboSupportGangType.Text.Trim() == "DEVELOPMENT")
                            {

                            }
                            else
                            {
                                for (int i = 0; i < lstSupportGangs.SelectedItems.Count; i++)
                                {
                                    DataRow dr = temp.NewRow();
                                    ...
                                    dr["SECTION"] = txtSelectedSection.Text.Trim();
                                    dr["PERIOD"] = txtPeriod.Text.Trim();
                                    dr["DAYGANG"] = txtSupportLinkDayShiftGang.Text.Trim();
                                    dr["GANG"] = lstSupportGangs.SelectedItems[i].ToString();
                                    dr["GANGTYPE"] = cboSupportGangType.Text.Trim();
                                    dr["SAFETYIND"] = cboSupportLinkSafetyInd.Text.Trim();
                                ...
                                TB.saveCalculations2(temp, Base.DBConnectionString, strDelete, "SUPPORTLINK");
                                evaluateSupportLink();

                                grdSupportLink.FirstDisplayedScrollingRowIndex = intRow;
                            }
                        ...
                        cboAutoDrillerDrilInd.Text.Trim().Length > 0)
                    {
                        if (Convert.ToInt32(cboAutoDrillerDrilInd.Text.ToString().Trim()) > 0)
                        {
                            ...
                            else
                            {
                                ...
                                if (txtAutoDWorkplace.Text.Trim().Contains("-"))
                                {
        ...
        private void grdMiners_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            foreach (string s in lstPrimaryKeyColumns)
            {
                ...
                {
                    dictPrimaryKeyValues.Add(s, grdMiners[s, e.RowIndex].Value.ToString().Trim());
                }
            }
            ...
            foreach (string s in lstTableColumns)
            {
                ...
                {
                    dictGridValues.Add(s, grdMiners[s, e.RowIndex].Value.ToString().Trim());
                }
            }
            #endregion
        }
            ...
            else
            {
                ...
                switch (tabInfo.SelectedTab.Name)
                {
                    ...
                    case "tabDrillPen":
                        ...
                        btnLock.Enabled = true;
                        btnPrint.Enabled = true;
                        extractPrimaryKey(DrillPen, "DRILLERPENALTY");
                        break;
        ...
        private int checkLock(string processToBeChecked)
        {
            ...
            catch
            {
                ...
                MessageBox.Show("No columns found in the execution of CHECKLOCK");
                return 0;
        ...
        private int checkLockInputProcesses()
        {
            ...
            catch
            {
                ...
                MessageBox.Show("No columns found in the execution of checkLockInputProcesses.");
                return 0;
            }
        ...
        private void grdSurvey_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //    cboSurveyWorkplace.Text = grdSurvey["WORKPLACE", e.RowIndex].Value.ToString().Trim();
            //    //cboSurveyWorkplaceType.Text = grdSurvey["WORKPLACETYPE", e.RowIndex].Value.ToString().Trim();
            //    //cboSurveySwpType.Text = grdSurvey["TYPE", e.RowIndex].Value.ToString().Trim();
            //    //txtSurveyActHigh.Text = grdSurvey["ACTUALHEIGHT", e.RowIndex].Value.ToString().Trim();
            //    //txtSurveyActWidth.Text = grdSurvey["ACTUALWIDTH", e.RowIndex].Value.ToString().Trim();
            //    txtSurveyCW.Text = grdSurvey["CHANNELWIDTH", e.RowIndex].Value.ToString().Trim();
            //    //txtSurveyDIP.Text = grdSurvey["DIP", e.RowIndex].Value.ToString().Trim();
            //    //txtSurveyLedgeFL.Text = grdSurvey["LEDGEFACELENGTH", e.RowIndex].Value.ToString().Trim();
            //    txtSurveyLedgeM2.Text = grdSurvey["LEDGESQM", e.RowIndex].Value.ToString().Trim();
            //    txtSurveyStopeM2.Text = grdSurvey["STOPESQM", e.RowIndex].Value.ToString().Trim();
            //    txtSurveyStopeWidth.Text = grdSurvey["STOPEWIDTH", e.RowIndex].Value.ToString().Trim();
            //    txtSurveySwpDist.Text = grdSurvey["SWEEPINGDISTANCE", e.RowIndex].Value.ToString().Trim();
            //    //txtSurveySwpM2.Text = grdSurvey["SWEEPINGSQM", e.RowIndex].Value.ToString().Trim();
            //    //txtSurveyFL.Text = grdSurvey["STOPEFACELENGTH", e.RowIndex].Value.ToString().Trim();
            //    //txtSurveyLedgeFL.Text = grdSurvey["LEDGEFACELENGTH", e.RowIndex].Value.ToString().Trim();
        ...
        private void grdEmplPen_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                cboEmplPenEmployeeName.Text = grdEmplPen["EMPLOYEE_NO", e.RowIndex].Value.ToString().Trim();
        ...
        private void grdLabour_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else
            {
                ...
                txtStrikeShifts.Text = grdLabour["LAPA_SHIFTS", e.RowIndex].Value.ToString().Trim();
        ...
        private void grdOffdays_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
                        ...
                        {  
                            cboOffDaysSection.Text = grdOffDays["SECTION", e.RowIndex].Value.ToString().Trim();
                            cboOffDaysGang.Text = grdOffDays["GANG", e.RowIndex].Value.ToString().Trim();
                            cboLapaShiftInd.Text = grdOffDays["LAPASHIFTIND", e.RowIndex].Value.ToString().Trim();
                            txtOffdate.Text = grdOffDays["OFFDAYVALUE", e.RowIndex].Value.ToString().Trim();
                        }
        ...
        {
            DataTable searchEmpl = TB.createDataTableWithAdapter(Base.DBConnectionString,
                                   "Select * from ClockedShifts where employee_no like '%" + txtSearchEmpl.Text.Trim() + "%'");
        ...
        private void grdSupportLink_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            Cursor.Current = Cursors.WaitCursor;
            ...
            else
            {
                ...
                txtSupportLinkDayShiftGang.Text = grdSupportLink["DAYGANG", e.RowIndex].Value.ToString().Trim();
                cboSupportGangType.Text = grdSupportLink["GANGTYPE", e.RowIndex].Value.ToString().Trim();
            }
            ...
            foreach (string s in lstPrimaryKeyColumns)
            {
                ...
                {
                    dictPrimaryKeyValues.Add(s, grdSupportLink[s, e.RowIndex].Value.ToString().Trim());
                }
            }
            ...
            foreach (string s in lstTableColumns)
            {
                ...
                {
                    dictGridValues.Add(s, grdSupportLink[s, e.RowIndex].Value.ToString().Trim());
                }
            }
            #endregion
        }
        ...
        private void grdGangLink_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else
            {
                ...
                //txtGangLinkContract.Text = grdGangLink["CONTRACT", e.RowIndex].Value.ToString().Trim();
                ...
                cboGangLinkGangType.Text = grdGangLink["GANGTYPE", e.RowIndex].Value.ToString().Trim(); //amp
            }
            #region Trigger output
            //load the CURRENT values into dictionaries before the update 
            // that was loaded in tabInfo_SelectedIndexChanged
        ...
        private void extractMeasuringDates()
        {
            ...
            {
                MessageBox.Show("No records on calender for the selected period and section");
            }
            ...
            switch (tabInfo.SelectedTab.Name)
            {
                case "tabAbnormal":
                        ...
                        txtAbnormalValue.Text.Trim().Length != 0)
                    {
                        ...
                        if (grdAbnormal[2, intRow].Value.ToString().Trim() != "XXX")
                        {
                            ...
                            foreach (string s in lstTableColumns)
                            {
                                if (dictGridValues[s] == grdAbnormal[s, intRow].Value.ToString().Trim())
                                {
                                ...
                                else
                                {
                                    ...
                                    writeAudit("ABNORMAL", "U - Update", s, dictGridValues[s], grdAbnormal[s, intRow].Value.ToString().Trim());
                ...
                case "tabMiners":
                        ...
                        txtMinersSafetyInd.Text.Trim().Length != 0)
                    {
                        ...
                        foreach (string s in lstTableColumns)
                        {
                            if (dictGridValues[s] == grdMiners[s, intRow].Value.ToString().Trim())
                            {
                            ...
                            {
                                //Write out to audit log
                                writeAudit("MINERS", "U - Update", s, dictGridValues[s], grdMiners[s, intRow].Value.ToString().Trim());
                ...
                case "tabLabour":
                        ...
                        && txtDrillerShifts.Text.Trim().Length > 0)
                    {
                        ...
                        string strStrikes = Convert.ToString(grdLabour["LAPA_SHIFTS", intRow].Value);
                        ...
                        strSQL = "Update bonusshifts set wagecode = '" + cboBonusShiftsWageCode.Text.Trim() +
                                 ...
                                 "' , Lapa_Shifts = '" + txtStrikeShifts.Text.Trim() +
                                 ...
                                 "' and Period = '" + grdLabour["Period", intRow].Value +                  //xxxxxxxxxxxxxxxx
                        ...
                        grdLabour["LAPA_SHIFTS", intRow].Value = txtStrikeShifts.Text.Trim();
                        ...
                        //move updated values to the dictionary.  Compare updated values with the old values and write trigger.
                        //The primary keys are loaded when the user clicks on the grid.
                        foreach (string s in lstTableColumns)
                        {
                ...
                case "tabGangLinking":
                        ...
                        cboGangLinkGang.Text.Trim().Length > 0)
                    {
                        ...
                        strSQL = "Update ganglink set  Workplace = '" + cboGangLinkWorkplace.Text.Trim() +
                                 ...
                                 "' , ShiftBoss = '" + cboGanglinkShiftbosses.Text.Trim() +
                        ...
                        strSQL = strSQL.Trim() + " Update ganglink set shiftboss = '" + cboGanglinkShiftbosses.Text.Trim() +
                                                                  "' where gang = '" + cboGangLinkGang.Text.Trim() + "'";
                        ...
                        grdGangLink["ShiftBoss", intRow].Value = cboGanglinkShiftbosses.Text.Trim();
                        ...
                        for (int i = 0; i <= grdGangLink.Rows.Count - 2; i++)
                        {
                            //string test = grdGangLink["Gang", i].Value.ToString().Trim();
                            if (grdGangLink["Gang", i].Value.ToString().Trim() == cboGangLinkGang.Text.Trim())
                            {
                                grdGangLink["Shiftboss", i].Value = cboGanglinkShiftbosses.Text.Trim();
                            }
                        }
                ...
                case "tabEmplPen":
                        ...
                        txtPenaltyValue.Text.Trim().Length != 0 && cboPenaltyInd.Text.Trim().Length != 0)
                    {
                        ...
                        if (cboEmplPenEmployeeName.Text.Contains("-"))
                        {
                            strName = cboEmplPenEmployeeName.Text.Substring(0, cboEmplPenEmployeeName.Text.IndexOf("-")).Trim();
                        }
                        else
                        {
                            strName = cboEmplPenEmployeeName.Text.Trim();
                        }
                        ...
                        if (grdEmplPen["EMPLOYEE_NO", intRow].Value.ToString().Trim() != "XXXXXXXXXXXX")
                        {
                            ...
                            grdEmplPen["Employee_No", intRow].Value = cboEmplPenEmployeeName.Text.Trim();
                ...
                case "tabSupportLink":
                          ...
                          txtSupportLinkDayShiftGang.Text.Trim().Length > 0)
                    {
                        ...
                        strSQL = "Update supportlink set  gangtype = '" + cboSupportGangType.Text.Trim() +
                                 "' , safetyind = '" + cboSupportLinkSafetyInd.Text.Trim() +
                                 "' where gang = '" + grdSupportLink["Gang", intRow].Value +
                                 "' and daygang = '" + grdSupportLink["Daygang", intRow].Value +
                                 "' and period = '" + grdSupportLink["Period", intRow].Value +    //xxxxxxxxxxxxxxxx
                                 "' and section = '" + grdSupportLink["Section", intRow].Value + //xxxxxxxxxxxxxxxx
                                 "' and gangtype = '" + grdSupportLink["Gangtype", intRow].Value + "'";

                        TB.InsertData(Base.DBConnectionString, strSQL);


                        grdSupportLink["SafetyInd", intRow].Value = cboSupportLinkSafetyInd.Text.Trim();
                        grdSupportLink["Gangtype", intRow].Value = cboSupportGangType.Text.Trim();

                        for (int i = 0; i <= grdSupportLink.Columns.Count - 1; i++)
                        {
                            grdSupportLink[i, intRow].Style.BackColor = Color.Lavender;
                        }
                        //move updated values to the dictionary.  Compare updated values with the old values and write trigger.
                        foreach (string s in lstTableColumns)
                        {
                            if (dictGridValues[s] == grdSupportLink[s, intRow].Value.ToString().Trim())
                            {
                            ...
                            {
                                //Write out to audit log
                                writeAudit("SUPPORTLINK", "U - Update", s, dictGridValues[s], grdSupportLink[s, intRow].Value.ToString().Trim());
            ...
            switch (tabInfo.SelectedTab.Name)
            {
                ...
                case "tabSupportLink":
                    ...
                    if (cboSupportGangType.Text.Trim().Length > 0 &&
                        txtSupportLinkDayShiftGang.Text.Trim().Length > 0)
                    {
                        strSQL = "delete from SupportLink where Daygang = '" + txtSupportLinkDayShiftGang.Text.Trim() +
                                 "' and GANG = '" + grdSupportLink["Gang", intRow].Value.ToString().Trim() +
                                 "' and gangtype = '" + grdSupportLink["Gangtype", intRow].Value.ToString().Trim() + "'";
                ...
                case "tabOffdays":
                    ...
                    {
                        strSQL = "delete from Offdays where gang = '" + cboOffDaysGang.Text.Trim() +
                                 "' and section = '" + cboOffDaysSection.Text.Trim() +
                                 "' and OffDayValue = '" + txtOffdate.Text.Trim() + 
                                 "' and Lapashiftind = '" + cboLapaShiftInd.Text.Trim() + "'";
        ...
        private void btnCostsheetPhase1_Click(object sender, EventArgs e)
        {
            ...
            btnx.Enabled = true;
            btnx.Text = "Run";
            TB.deleteProcess(Base.AnalysisConnectionString, Base.DBName);
            TB.deleteAllExcept(Base.DBConnectionString, "Monitor");
            Calcs("Miners", "Miners10", "Y");
            Calcs("Miners", "Miners20", "N");
            Calcs("Miners", "Miners30", "N");
            Calcs("Miners", "Miners40", "N");
            Calcs("Miners", "Miners50", "N");
            Calcs("Miners", "Miners60", "N");

            btnMinersCalc.BackColor = Color.Orange;

            TB.updateStatusFromArchive(Base.DBConnectionString, "N", "Miners10", txtSelectedSection.Text.Trim());
            this.Cursor = Cursors.Arrow;
        ...
        private void CalcCostSheetEmployees()
        {
            ...
            if (intCheckLocks == 0)
            {
                ...
                //btnTeamPhase1.BackColor = Color.LightGreen;

                //Application.DoEvents();
                //Base.UpdateStatus(Base.DBConnectionString, "Y", "Costsheet Calc Process", "Costsheet Calc Process - Phase 1", txtPeriod.Text.Trim(), txtSelectedSection.Text.Trim());
                //Base.UpdateStatus(Base.DBConnectionString, "Y", "Header", "CostSheet Calc Process", txtPeriod.Text.Trim(), txtSelectedSection.Text.Trim());
                //Application.DoEvents();
                //evaluateStatus();
            }
            else
        ...
        private void startCalcProcess()
        {
            ...
            TB.deleteAllExcept(Base.DBConnectionString, "Monitor");
            //Check if the earnings tables exist.  If not, create the table.
            //extractPrimaryKey(GangLink, "GANGLINK");
            //This all happens in the CALCS method.
            //Calcs("Production", "ProductionEarn999", "N");
            Calcs("BonusShifts", "BonusShiftsEARN09", "Y");
            Calcs("GangLink", "GanglinkEARN10", "Y");
            Calcs("GangLink", "GanglinkEARN12", "N");
            ...
            Calcs("SupportLink", "SupportLinkEARN10", "Y");
            Calcs("SupportLink", "SupportLinkEARN20", "N");
            ...
            Calcs("BonusShifts", "BonusShiftsEARN80", "N");
            Calcs("BonusShifts", "BonusShiftsEARN90", "N");
            Calcs("Miners", "MinersEARN9", "Y");
            Calcs("Miners", "MinersEARN10", "N");
            Calcs("Miners", "MinersEARN15", "N");
            Calcs("Miners", "MinersEARN20", "N");
            Calcs("Miners", "MinersEARN30", "N");
            Calcs("Miners", "MinersEARN40", "N");
            Calcs("Miners", "MinersEARN50", "N");
            Calcs("Miners", "MinersEARN60", "N");
            Calcs("Miners", "MinersEARN70", "N");
            Calcs("Exit", "Exit", "N");
            ...
            TB.updateStatusFromArchive(Base.DBConnectionString, "N", "GanglinkEARN40", txtSelectedSection.Text.Trim(), BusinessLanguage.Period.Trim(), "");
            TB.updateStatusFromArchive(Base.DBConnectionString, "N", "SupportLinkEARN20", txtSelectedSection.Text.Trim(), BusinessLanguage.Period.Trim(), "");
            ...
            TB.updateStatusFromArchive(Base.DBConnectionString, "N", "MinersEARN60", txtSelectedSection.Text.Trim(), BusinessLanguage.Period.Trim(), "");
        ...
        private void btnBonusShiftsCalcs_Click(object sender, EventArgs e)
        {
            ...
            TB.deleteProcess(Base.AnalysisConnectionString, Base.DBName);
            TB.deleteAllExcept(Base.DBConnectionString, "Monitor");
            Calcs("BonusShifts", "BonusShifts0", "Y");   //amp
            Calcs("BonusShifts", "BonusShifts1", "N");
            Calcs("BonusShifts", "BonusShifts2", "N");
            Calcs("BonusShifts", "BonusShifts3", "N");
            Calcs("BonusShifts", "BonusShifts10", "N");
            Calcs("Miners", "Miners1", "Y");
            Calcs("Miners", "Miners2", "N");
            Calcs("Miners", "Miners3", "N");
            Calcs("Miners", "Miners10", "N");

            btnGangLinkCalcs.BackColor = Color.Orange;
            btnMinersCalc.BackColor = Color.LightBlue;
            btnBonusShiftsCalcs.BackColor = Color.LightBlue;

            TB.updateStatusFromArchive(Base.DBConnectionString, "N", "BonusShifts10", txtSelectedSection.Text.Trim());
        ...
        private void btnTeamPrint_Click(object sender, EventArgs e)
        {
            ...
            mm.StartReport("Devtm4000TeamPortrait");
            this.Cursor = Cursors.Arrow;
        ...
        private void printCostsheet_Click(object sender, EventArgs e)
        {
            ...
            mm.Init(strMetaReportCode);
            //mm.ProjectsPath = "c:\\icalc\\Harmony\\Tshepong\\" + strServerPath + "\\REPORTS\\";
            mm.StartReport("Devtm4000CostSheet");
            this.Cursor = Cursors.Arrow;
        }
        ...
        private void btnSupportAuth_Click(object sender, EventArgs e)
        {
            ...
            //mm.Init(strMetaReportCode);
            ////mm.ProjectsPath = "c:\\icalc\\Harmony\\Tshepong\\" + strServerPath + "\\REPORTS\\";
            //mm.StartReport("DEVTMSupportAuthorization");
            //this.Cursor = Cursors.Arrow;

            metareportAutoW3Parameter("Section_value", "GangType", "Period", txtSelectedSection.Text.Trim(), "Tramming", Base.Period, "", "DEVTMSupportAuthorization");
        }
        ...
        private void btnRiggingTeamAuth_Click(object sender, EventArgs e)
        {
            ...
            mm.Init(strMetaReportCode);
            //mm.ProjectsPath = "c:\\icalc\\Harmony\\Tshepong\\" + strServerPath + "\\REPORTS\\";
            mm.StartReport("Devtm4000RiggingAuthorization");
            this.Cursor = Cursors.Arrow;
        }
        ...
        private void btnEquippingTeamAuth_Click(object sender, EventArgs e)
        {
            ...
            mm.Init(strMetaReportCode);
            //mm.ProjectsPath = "c:\\icalc\\Harmony\\Tshepong\\" + strServerPath + "\\REPORTS\\";
            mm.StartReport("Devtm4000EquippingAuthorization");
            this.Cursor = Cursors.Arrow;
        }
        ...
        private void btnHaulageTeamAuth_Click(object sender, EventArgs e)
        {
            ...
            mm.Init(strMetaReportCode);
            //mm.ProjectsPath = "c:\\icalc\\Harmony\\Tshepong\\" + strServerPath + "\\REPORTS\\";
            mm.StartReport("Devtm4000HaulageAuthorization");
            this.Cursor = Cursors.Arrow;
        }
        ...
        private void btnDrillerPrint_Click(object sender, EventArgs e)
        {
            ...
            mm.Init(strMetaReportCode);
            //mm.ProjectsPath = "c:\\icalc\\Harmony\\Tshepong\\" + strServerPath + "\\REPORTS\\";
            mm.StartReport("Devtm4000Drillers");
            this.Cursor = Cursors.Arrow;
        ...
        private void btnDrillerAuth_Click(object sender, EventArgs e)
        {
            ...
            mm.Init(strMetaReportCode);
            //mm.ProjectsPath = "c:\\icalc\\Harmony\\Tshepong\\" + strServerPath + "\\REPORTS\\";
            mm.StartReport("Devtm4000DrillerAuth");
            this.Cursor = Cursors.Arrow;
        }
        ...
        private void cboNames_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (Clocked.Rows.Count > 0)
            {
                ...
                catch
                {
                    ...
                    MessageBox.Show("No employees in clockedshifts.");
                }
            }
        ...
        private void scrTeamD_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void txtSearchEmplyNr_TextChanged(object sender, EventArgs e)
        {
            ...
            searchEmplName = "";
            searchEmplGang = "";
            searchBonus(searchEmplNr, searchEmplName, searchEmplGang, grdLabour); //Calls the method
        ...
        {
            DataTable dt = Base.dataPrintFormulasBonusShifts(Base.AnalysisConnectionString, Base.DBName, "Sectionearnings");
            if (dt.Rows.Count > 0)
            {
                printHTML(dt, "Formulas on SECTIONEARNINGS");
            }
            else
            {
                MessageBox.Show("No records available to print", "", MessageBoxButtons.OK);
        ...
        //{
        //    //Setting the names to be send to the method
        //    grdSupportLink.Sort(grdSupportLink.Columns["EMPLOYEE_NO"], ListSortDirection.Ascending);
        //    searchEmplNr2 = txtSearchEmplyNr2.Text.ToString();
        //    searchEmplGang2 = "";
        //    searchBonus(searchEmplNr2, "", searchEmplGang2, grdSupportLink); //Calls the metod
        ...
        //{
        //    //Setting the names to be send to the method
        //    grdSupportLink.Sort(grdSupportLink.Columns["GANG"], ListSortDirection.Ascending);
        //    searchEmplNr2 = "";
        ...
        private void grdAbnormal_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            foreach (string s in lstPrimaryKeyColumns)
            {
                ...
                {
                    dictPrimaryKeyValues.Add(s, grdAbnormal[s, e.RowIndex].Value.ToString().Trim());
                }
            }
            ...
            foreach (string s in lstTableColumns)
            {
                ...
                {
                    dictGridValues.Add(s, grdAbnormal[s, e.RowIndex].Value.ToString().Trim());
                }
            }
            #endregion
        ...
        private void btnMiners_Click(object sender, EventArgs e)
        {
            ...
            if (Miners.Rows.Count == 0)
            {

                Miners = TB.extractMinersLink(Base.DBConnectionString, txtSelectedSection.Text.Trim(), "DEVELOPMENT");
                TB.saveCalculations2(Miners, Base.DBConnectionString, strWhere, "MINERS");
            }
            else
            {
                DialogResult result = MessageBox.Show("Do you want to redo the Miners for section: " + txtSelectedSection.Text.Trim(), "Question", MessageBoxButtons.YesNo);
                switch (result)
                {
                    case DialogResult.Yes:
                        TB.InsertData(Base.DBConnectionString, "Delete from Miners where section = '" + txtSelectedSection.Text.Trim() + "'");
                        Miners = TB.extractMinersLink(Base.DBConnectionString, txtSelectedSection.Text.Trim(), "DEVELOPMENT");
                        TB.saveCalculations2(Miners, Base.DBConnectionString, " where employee_no = '999'", "MINERS");
                        break;
        ...
        private void cboMinersEmpName_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (Clocked.Rows.Count > 0)
            {
                IEnumerable<DataRow> query1 = from locks in Labour.AsEnumerable()
                                              where locks.Field<string>("EMPLOYEE_NAME").TrimEnd() == cboMinersEmpName.Text.Trim()
                                              select locks;
                ...
                {
                    MessageBox.Show("Employee no: " + cboMinersEmpName.Text.Trim() + " was not found in BonusShifts.");
                }
            }
            ...
            if (temp.Rows.Count > 0)
            {
                ...
                txtADTeamShifts.Text = temp.Rows[0]["SHIFTS_WORKED"].ToString().Trim();
                txtPayShifts.Text = temp.Rows[0]["SHIFTS_WORKED"].ToString().Trim();
                txtAwops.Text = temp.Rows[0]["AWOP_SHIFTS"].ToString().Trim();
            }
            else
            {
                txtPayShifts.Text = "1000";
                txtAwops.Text = "1000";
            }
        ...
        private void cboColumnShow_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Selected filter is invalid.");
            }
        }
        ...
        private void btnUnLink_Click(object sender, EventArgs e)
        {
            ...
            DataTable unlink = new DataTable();
            unlink.Columns.Add("BUSSUNIT");
            unlink.Columns.Add("MININGTYPE");
            unlink.Columns.Add("BONUSTYPE");
            unlink.Columns.Add("SECTION");
            unlink.Columns.Add("PERIOD");
            unlink.Columns.Add("GANG");
            unlink.AcceptChanges();

            if (lstGangs.Items.Count > 0)
            {
                for (int i = 0; i <= lstGangs.Items.Count - 1; i++)
                {
                    DataRow dr = unlink.NewRow();
                    dr["BUSSUNIT"] = BusinessLanguage.BussUnit;
                    dr["MININGTYPE"] = BusinessLanguage.MiningType;
                    dr["BONUSTYPE"] = BusinessLanguage.BonusType;
                    dr["SECTION"] = txtSelectedSection.Text.Trim();
                    dr["PERIOD"] = txtPeriod.Text.Trim();
                    dr["GANG"] = lstGangs.Items[i].ToString();
                    unlink.Rows.Add(dr);

                }

                if (unlink.Rows.Count > 0)
                {
                    printHTML(unlink, "UnlinkGang Report");
                }
                else
                {
                    MessageBox.Show("No records available to print", "", MessageBoxButtons.OK);
        ...
        private void createUnlinkList()
        {
            //Create a backuplist of lstGangs
            //lstBackup will contain all the gangs for the section as listbox lstGang does.
            ...
            if (Labour.Rows.Count > 0)
            {
                ...
                //Extract the gang numbers of the gangs already linked
                List<string> lstTemp = new List<string>();
                lstTemp = TB.loadDistinctValuesFromColumn(GangLink, "Gang");

                //Clear lstGang and then add only the gang numbers that are on 
                //lstBackup but not in lstTemp
                lstGangs.Items.Clear();
                ...
                for (int i = 0; i <= lstBackup.Count - 1; i++)
                {
                    ...
                    else
                    {
                        ...
                        lstGangs.Items.Add(lstBackup[i].ToString());
                    }
                }
        ...
        private void btnUnlinkList_Click(object sender, EventArgs e)
        {
            createUnlinkList();
        }
        ...
        private void cboColumnValues_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Selected filter on values is invalid.");
            }
        ...
        private void cboAutoDrillerDrilInd_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cboAutoDrillerDrilInd.Text.Trim() == "1")
            {
                txtAutoDrilShifts.Enabled = true;
                txtAutoDrilShifts.Focus();
            }
        }

        private void grdSupportLink_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                autoSizeGrid(grdSupportLink);
            }
        }
        ...
        private void btnSupportLinkUnlinkList_Click(object sender, EventArgs e)
        {
            //Create a backuplist of lstGangs
            //lstBackup will contain all the gangs for the section as listbox lstGang does.
            ...
            if (Labour.Rows.Count > 0)
            {
                //Extract all possible gangs for the section
                List<string> lstBackup = TB.loadDistinctValuesFromColumn(Labour, "GANG");


                //Extract the gang numbers of the gangs already linked on ganglinking
                List<string> lstTemp = new List<string>();
                lstTemp = TB.loadDistinctValuesFromColumn(GangLink, "Gang");

                //Extract the gang numbers of the gangs already linked on supportlinking
                List<string> lstSupportTemp = new List<string>();
                lstSupportTemp = TB.loadDistinctValuesFromColumn(SupportLink, "Gang");

                //Clear lstSupportGang and then add only the gang numbers that are on 
                //lstBackup but not in lstTemp(ganglinking) AND not on lstSupportTemp(Supportlinking)
                lstSupportGangs.Items.Clear();
                ...
                for (int i = 0; i <= lstBackup.Count - 1; i++)
                {
                    ...
                    {
                        if (lstSupportTemp.Contains(lstBackup[i].ToString()))
                        {

                        }
                        else
                        {
                            lstSupportGangs.Items.Add(lstBackup[i].ToString());
                        }
                    }
                }

                lstSupportGangs.Refresh();
            }
            else
            {
                MessageBox.Show("No records in Bonusshifts.  Please re-import clocked shifts.", "Information", MessageBoxButtons.OK);
        ...
        private List<string> extractDayShiftGangs(string Gang)
        {
            ...
            if (SupportLink.Rows.Count > 0)
            {
                IEnumerable<DataRow> query1 = from locks in SupportLink.AsEnumerable()
                                              where locks.Field<string>("GANG").Trim() == Gang
                                              select locks;
        ...
        private List<string> extractGangWorkplaces(string Gang)
        {
            ...
            if (GangLink.Rows.Count > 0)
            {
                IEnumerable<DataRow> query1 = from locks in GangLink.AsEnumerable()
                                              where locks.Field<string>("GANG").Trim() == Gang
                                              where locks.Field<string>("PERIOD").Trim() == BusinessLanguage.Period
                                              select locks;
        ...
        private void grdProduction_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                autoSizeGrid(grdProduction);
            }
        }

        private void grdAbnormal_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                autoSizeGrid(grdAbnormal);
        ...
        private void grdEmplPen_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                autoSizeGrid(grdEmplPen);
            }
        }

        private void btnSetOffdays_Click(object sender, EventArgs e)
        {
            openTab(tabOffdays);
        }
        ...
        {
            cboMinersEmpName.Text = cboEmplPenEmployeeName.SelectedItem.ToString().Trim();
        ...
        private void graphTransfer_Click(object sender, EventArgs e)
        {

            string ServerPath = "z:\\Harmony\\Tshepong\\Production\\Data\\adteam_" + BusinessLanguage.Period.Trim() + ".xls";
            string FilePath_XLSX = "C:\\iCalc\\Harmony\\Tshepong\\" + strServerPath + "\\Data\\adteam_" + BusinessLanguage.Period.Trim() + ".xlsx";

            //Select the from database name
            OpenFileDialog openFile1 = new OpenFileDialog();
            openFile1.InitialDirectory = "C:\\icalc\\harmony\\tshepong\\Publish";
            openFile1.Filter = @"FrontEnd|*.xbap";
        ...
        private void extractGangLinkData()
        {
            string strSQL = "select distinct '" + BusinessLanguage.BussUnit + "' as BUSSUNIT,'" + BusinessLanguage.MiningType + "' as MININGTYPE,'" +
                            ...
                            " from Survey " +
                            " where Section = '" + txtSelectedSection.Text.Trim() + "' AND PERIOD = '" + BusinessLanguage.Period +
                            "' and workplace not in(select distinct workplace from ganglink where period = '" + BusinessLanguage.Period + "')";
        ...
        private void btnRefreshSupportLink_Click(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;
            extractSupportLinkData();
            this.Cursor = Cursors.Arrow;
            MessageBox.Show("Supportlinking has been refreshed and the extra daygangs inserted.", "Information", MessageBoxButtons.OK);
            this.Cursor = Cursors.Arrow;
        }

        private void extractSupportLinkData()
        {
            //Get the layout of the ganglink file.
            DataTable temp = new DataTable();
            temp = SupportLink.Copy();
            ...
            List<string> Gangs = TB.loadDistinctValuesFromColumn(GangLink, "GANG");
            List<string> lstDayGangs = TB.loadDistinctValuesFromColumn(SupportLink, "DAYGANG");

            foreach (string s in Gangs)
            {
                if (s.Trim().Length == 10)
                {
                    string test = s.Substring(8, 1);
                    if (s.Substring(8, 1) == "R")
                    {
                        if (lstDayGangs.Contains(s.Trim()))
                        {
                        }
                        else
                        {
        ...
        private void btnDrillerCalcs_Click(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;
            btnx.Visible = true;
            btnx.Enabled = true;
            ...
            TB.updateStatusFromArchive(Base.DBConnectionString, "N", "DrillersEARN30", txtSelectedSection.Text.Trim(), BusinessLanguage.Period.Trim(), "");
            TB.updateStatusFromArchive(Base.DBConnectionString, "N", "Exit", txtSelectedSection.Text.Trim(), BusinessLanguage.Period.Trim(), "");
        ...
        private void btnTrammingPrint_Click(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;
            MetaReportRuntime.App mm = new MetaReportRuntime.App();
            mm.Init(strMetaReportCode);
            // mm.ProjectsPath = "c:\\icalc\\Harmony\\Tshepong\\" + strServerPath + "\\REPORTS\\";
            mm.StartReport("Devtm4000Tramming");
            this.Cursor = Cursors.Arrow;
        }

        private void btnRiggingPrint_Click(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;
            MetaReportRuntime.App mm = new MetaReportRuntime.App();
            mm.Init(strMetaReportCode);
            // mm.ProjectsPath = "c:\\icalc\\Harmony\\Tshepong\\" + strServerPath + "\\REPORTS\\";
            mm.StartReport("Devtm4000Rigging");
            this.Cursor = Cursors.Arrow;
        }

        private void btnEquippingPrint_Click(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;
            MetaReportRuntime.App mm = new MetaReportRuntime.App();
            mm.Init(strMetaReportCode);
            // mm.ProjectsPath = "c:\\icalc\\Harmony\\Tshepong\\" + strServerPath + "\\REPORTS\\";
            mm.StartReport("Devtm4000Equipping");
            this.Cursor = Cursors.Arrow;
        }

        private void btnLockDrillPen_Click(object sender, EventArgs e)
        {
            openTab(tabDrillPen);
        }
